...
3. Задать владельцу вопросы:
   a. "У тебя уже есть файл менеджера проектов (PROJECTS_TASKS.md)?"
      - Если ДА → спросить SSH доступ, подключиться, использовать существующий
      - Если НЕТ → создать локально в workspace
   b. "Создать веб-версию проект менеджера?"
      - Если на центральном сервере (pm_type = remote) уже ЕСТЬ веб-версия → пропустить, не создавать дубликат
      - Если central server ЕСТЬ но веб-версии НЕТ на центральном → спросить: "Создать веб-версию на центральном сервере?"
      - Если центрального сервера НЕТ (pm_type = local) → спросить: "Создать веб-версию на этом сервере?"
      - Если ДА на любой из вопросов:
        1. Создать шаблон веб-версии (HTML/JS, читает PROJECTS_TASKS.md + TZ.md)
        2. Установить nginx (или использовать python -m http.server для простоты)
        3. Открыть порт
        4. Сообщить: "✅ Веб-версия доступна: http://IP:ПОРТ"
        5. Записать URL в CONFIG.md: `web_url: http://IP:ПОРТ`
      - Если НЕТ → пропустить
   c. "Есть другие серверы с агентами?" → если ДА, спросить SSH
   d. "GitHub настроен?" → если НЕТ, github_enabled = false
   e. "Твой Telegram ID?" → записать
   f. "Есть оркестратор?" → если НЕТ, пропустить
4. Создать RULES.md с 6 дефолтными правилами
...
7. **Если есть центральный сервер (pm_type = remote):**
   ...
   c. Показать владельцу: "Я нашёл N агентов: [список]"
   d. Добавить СЕБЯ в AGENT_NETWORK.md
